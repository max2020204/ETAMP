...
namespace ETAMPManagment.Encryption
{
    ...
    public class EciesEncryptionService : IEciesEncryptionService
    {
        ...
        public virtual string Encrypt(string message)
        {
            ...
            byte[] encryptedMessage = _encryptionService!.Encrypt(Encoding.UTF8.GetBytes(message), _keyExchanger!.GetSharedSecret()!, _encryptionService.IV);
        ...
        public virtual string Decrypt(string encryptedMessageBase64)
        {
            ...
            byte[] encryptedMessage = Base64UrlEncoder.DecodeBytes(encryptedMessageBase64);

            byte[] decryptedMessage = _encryptionService!.Decrypt(encryptedMessage, _keyExchanger.GetSharedSecret()!, _encryptionService.IV!);
