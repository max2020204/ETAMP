...
namespace ETAMPManagment.Validators.Interfaces
{
    ...
    public interface IETAMPValidator
    {
        ...
        /// <returns>A task that represents the asynchronous operation, yielding true if the ETAMP token is valid; otherwise, false.</returns>
        Task<bool> ValidateETAMP(ETAMPModel etamp, string audience, string issuer, ECDsaSecurityKey tokenSecurityKey);

        /// <summary>
        /// Validates an ETAMP token for structure and signature, and checks the token's lifetime using a specified ECDsa security key.
        /// </summary>
        /// <param name="etamp">The ETAMP token model to be validated.</param>
        /// <param name="tokenSecurityKey">The ECDsa security key used for token signature validation.</param>
        /// <returns>A task that represents the asynchronous operation, yielding true if the ETAMP token is valid; otherwise, false.</returns>
        ...
        /// <summary>
        /// Performs a lightweight validation of an ETAMP token, focusing primarily on structure and signature checks using a specified ECDsa security key.
        /// </summary>
        /// <param name="etamp">The ETAMP token model to be validated.</param>
        /// <param name="tokenSecurityKey">The ECDsa security key used for validating the token's signature.</param>
        /// <returns>A task that represents the asynchronous operation, yielding true if the ETAMP token's structure and signature are valid; otherwise, false.</returns>
