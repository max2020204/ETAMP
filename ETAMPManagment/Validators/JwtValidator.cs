...
public sealed class JwtValidator : IJwtValidator
{
    ...
    public ValidationResult IsValidJwtToken(string token)
    {
        ...
        try
        {
            ...
            if (headerData == null || !headerData.TryGetValue("typ", out var value) || value != "ETAMP")
    ...
    public async Task<ValidationResult> ValidateLifeTime(string token, ECDsaSecurityKey securityKey)
    {
        ...
        try
        {
            var result = await _jwtSecurityTokenHandler.ValidateTokenAsync(token, GetValidationParameters(securityKey));
            return await Task.FromResult(new ValidationResult(result.IsValid,
                result.IsValid ? null : "Token's lifetime validation failed."));
        }
    ...
    /// <summary>
    ///     Asynchronously validates a JWT token, including its lifetime, audience, and issuer claims, using a specified ECDsa
    ///     security key.
    /// </summary>
    /// <param name="token">The JWT token to validate.</param>
    ...
    ///     If the token passes this initial check, it then proceeds to a comprehensive validation against the provided
    ///     audience, issuer, and signature.
    ///     Specific checks include validating the token's lifetime, audience, and issuer claims, as well as verifying the
    ///     token's signature using the provided ECDsa security key.
        ...
        ECDsaSecurityKey securityKey)
    {
        ...
        {
            var result =
                await _jwtSecurityTokenHandler.ValidateTokenAsync(token,
                    GetValidationParameters(securityKey, audience, issuer));
            return await Task.FromResult(new ValidationResult(result.IsValid,
                result.IsValid ? null : "Invalid JWT token claims."));
        }
        catch (SecurityTokenExpiredException ex)
        {
    ...
    private TokenValidationParameters GetValidationParameters(ECDsaSecurityKey issuerSigningKey,
        string? validAudience = null, string? validIssuer = null)
    {
