...
public class DeflateCompressionServiceTests
{
    ...
    private readonly Mock<ILogger<DeflateCompressionService>> _loggerMock;
    private readonly DeflateCompressionService _sut;

    public DeflateCompressionServiceTests()
    {
        _fixture = new Fixture();
        _loggerMock = new Mock<ILogger<DeflateCompressionService>>();
        _sut = new DeflateCompressionService(_loggerMock.Object);
    }
    ...
    public async Task DecompressStream_ValidInput_ReturnsDecompressedStream()
    {
        ...
        await using (var compressor = new DeflateStream(compressedStream, CompressionMode.Compress, true))
        {
            await compressor.WriteAsync(inputData, 0, inputData.Length);
        }
