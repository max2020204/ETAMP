...
public interface ISignatureValidator : IInitialize
{
    ...
    /// </summary>
    /// <typeparam name="T">The type of the token contained within the ETAMP model, which must derive from the <see cref="Token"/> class.</typeparam>
    /// <param name="etamp">The ETAMP model containing the message and token to be validated.</param>
    /// <returns>A <see cref="ValidationResult"/> containing the result of the validation, indicating whether the signature is valid and any associated error or exception details.</returns>
