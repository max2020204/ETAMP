...
public interface IECIESEncryptionManager
{
    ...
    /// <returns>A task representing the asynchronous operation. The task result contains the encrypted data as a UTF-8 encoded string.</returns>
    ...
    /// <returns>A task representing the asynchronous encryption operation. The result contains the encrypted data as a base64-encoded string.</returns>
    Task<byte[]> EncryptAsync(byte[] data, ECDiffieHellman privateKey, ECDiffieHellmanPublicKey publicKey);

    /// <summary>
    /// Asynchronously decrypts a string using ECIES (Elliptic Curve Integrated Encryption Scheme) with the specified private and public keys.
    /// </summary>
    ...
    /// <returns>A task that represents the asynchronous decryption operation. The task result contains the decrypted data as a byte array.</returns>
